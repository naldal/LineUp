...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('.scoreVisited'+num).find('.ii #red').css('max-width',width).css('width',width);
		$('.scoreVisited'+num).find('#star_form span').css('width',width/10).css('height',height);
		$('.scoreVisited'+num).find('#star_form img').attr('style','width:'+width+'px !important');
		if(cursor_stat==0){
		...
		if(cursor_stat==2){
			$('.scoreVisited'+num).find('.ii').attr('style','height:'+height+'px !important; width:'+width/10*score+'px;');
				$('.ii #red').attr('src','../img/노란별.png');
		}
		if(cursor_stat==3){
