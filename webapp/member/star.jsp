...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('.score'+num).find('.ii #red').css('max-width',width).css('width',width);
		$('.score'+num).find('#star_form span').css('width',width/10).css('height',height);
		$('.score'+num).find('#star_form img').attr('style','width:'+width+'px !important');
		if(cursor_stat==0){
		...
		if(cursor_stat==1){
			$('.score').eq(num).find('.ii #red').css('max-width',width).css('width',width);
			$('.score').eq(num).find('#star_form span').css('width',width/10).css('height',height);
			$('.score').eq(num).find('#star_form img').attr('style','width:'+width+'px !important');
		...
		if(cursor_stat==4){
			$('.score'+num).find('.ii').attr('style','height:'+height+'px !important; width:'+width/10*score+'px;');
			$('.score'+num+' .ii #red').attr('src','../img/노란별.png');
		}
